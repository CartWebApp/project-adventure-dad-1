...
export function pixelator(level = 2) {
    ...
    const pixelator_ctx = /** @type {CanvasRenderingContext2D} */ (
        pixelator.getContext('2d')
    );
...
export class InterpolatingDoubleTreeMap {
    ...
    get(key) {
        ...
        const lower_half = this.#entries
            .filter(([k]) => k < key)
            .toSorted(([a], [b]) => a - b);
        const upper_half = this.#entries
            .filter(([k]) => k > key)
            .toSorted(([a], [b]) => a - b);
    ...
    *entries() {
    ...
    get(x, y) {
        ...
        for (const e of this.#entities) {
            const { x, y, entity } = e;
            const points = entity.outline.map(point => ({
                x: point.x + x,
                y: point.y + y
            }));
        ...
        for (const entity of filtered) {
            if (
                highest === null ||
                entity.entity.layer > highest.entity.layer
            ) {
                highest = entity;
            }
        }
